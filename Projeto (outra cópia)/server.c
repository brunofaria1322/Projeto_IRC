...
void handleUDP(char *file, char*type, int port) {
  ...
  if ( bind(sockfd, (struct sockaddr *)&serv_addr,  sizeof(serv_addr)) < 0 ) {
	...
	//send filesize
	sleep(1);
	send_int_UDP(filesize, sockfd, serv_addr);
	printf ("sent\n");
	sleep(3);
	int n_sent,  n_received, left_size=filesize;
	//Encripted
	...
	//Not Encripted
		...
		while(left_size>0){
			...
			n_sent=sendto(sockfd, (const char *)stream, n_received*sizeof(unsigned char),0, (const struct sockaddr *) &serv_addr, sizeof(serv_addr));
			memset(stream, 0, sizeof(stream));
